...
export default class MovieInfo extends Component {
  ...
  render() {
    const img_src = "http://image.tmdb.org/t/p/w92" + this.props.info.api_data.poster_path;
    return (
      <div className='movie-info'>
        <img className='movie-info__poster' src={img_src} />
        <div className='movie-info__content'>
          <h3>{this.props.info.api_data.title} <small className='color-grey'>({this.datestring2year(this.props.info.api_data.release_date)})</small></h3>
          ...
          <p>{this.props.info.api_data.overview}</p>
        </div>
      </div>
    );
  }
